#include "day4_hw1/imu_watchdog.hpp"
...
void ImuWatchdog::on_state_timer()
{
  ...
  if (state_request_pending_) {
    return; //이전 응답을 아직 못 받았으면 새로 보내지 않음
  }
...
void ImuWatchdog::on_sensor_state(rclcpp::Client<GetState>::SharedFuture future)
{
  state_request_pending_ = false; //다음 주기에 다시 물어볼 수 있게 함
  RCLCPP_INFO(
    get_logger(), "fake_imu state: %s",
    future.get()->current_state.label.c_str()); //executor가 나중에 실행해 주는 응답 콜백
}
